...
Classification always uses `language` (the message's actual language) for
accuracy -- checking Gujarati text against English patterns would miss
matches, same failure mode we found and fixed in /analyze-call. The
explanation/alert text, however, is generated in `output_language` if
provided (falls back to `language` otherwise), so a user whose app is set
to English can still get an English explanation for a Gujarati message.
"""
...
def analyze_message(request: AnalyzeMessageRequest) -> AnalyzeResponse:
    ...
    output_language = request.output_language or request.language

    explanation = generate_explanation(
        category=category, risk_percent=risk_percent, language=output_language
    )

    trigger_alert = compute_trigger_alert(risk_percent)
    alert_message = generate_alert_message(category_str, output_language) if trigger_alert else ""
